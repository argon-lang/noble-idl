...
abstract class ScalaBackendBase private[backend] extends Backend {
...
object ScalaBackendBase {
  ...
  private[backend] def buildPackageMapping[V](mapping: Map[String, V]): Map[PackageName, V] =
    mapping
      .view
